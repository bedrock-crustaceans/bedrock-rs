...
impl_block_data! {
    ...
    /// only — its name as a string, or its discriminant as an integer — so it
    /// cannot express "every variant has its own payload type, chosen by a
    /// sibling key", which is what `#[serde(tag = "id")]` did. The dispatch is
    /// generated by hand instead, as [`Self::from_value`]/[`Self::to_value`].
...
/// nested under a key of their own — `#[serde(flatten)]` over a
/// `#[serde(tag = "id")]` enum, before. Neither has a facet equivalent nbtx
/// honours, so the split is done by hand in [`Self::from_value`].
#[derive(Debug, Clone, PartialEq)]
