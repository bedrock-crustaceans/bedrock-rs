...
use crate::{ffi, db::{Buffer, Database}};
...
impl<'k, 'db> KvRef<'k, 'db> {
    ...
    pub fn key(&self) -> Buffer<'static> {
        ...
        let result = unsafe {
            ffi::bedrockrs_iter_key(self.iter.as_ptr())
        };

        // something must go very wrong...
        assert!(!result.data.is_null(), "iterator key pointer was null, this is a bug.");
        ...
        unsafe {
            let slice = std::slice::from_raw_parts_mut(result.data.cast::<u8>(), result.size as usize);
    ...
    pub fn value(&self) -> Buffer<'static> {
        ...
        // of the iterator. This means the iterator must be valid and this method is safe to call.
        let result = unsafe {
            ffi::bedrockrs_iter_value(self.iter.as_ptr())
        };

        assert!(!result.data.is_null(), "iterator value pointer was null, this is a bug.");
...
impl<'db> Keys<'db> {
    pub fn new(db: &'db Database) -> Self {
        let result = unsafe {
            ffi::bedrockrs_iter_new(db.as_ptr())
        };
...
impl<'k, 'db> Iterator for &'k mut Keys<'db> {
    ...
    fn next(&mut self) -> Option<KvRef<'k, 'db>> {
        ...
        let valid = unsafe {
            ffi::bedrockrs_iter_valid(self.iter.as_ptr())
        };
        
        self.index += 1;
        valid.then_some(KvRef {
            iter: self.iter, _marker: PhantomData
        })
    }
}
